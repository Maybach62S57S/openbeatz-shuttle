...
function ReturnsTab({ setup, dyn, day, updateDyn, by, onErr, onAssign, onWhatsApp, onEdit, onNewReturn }) {
  ...
  // Rückfahrt = vom Festival weg (Typ return oder fromId festival)
  ...
  // Fahrer am Festival / frei (Kandidaten für Rückfahrten)
  ...
  const Tile = ({ r, big }) => {
    ...
    return (
      <div className={`rounded-2xl border p-4 ${issue ? "border-red-500/50 bg-red-500/10" : r.assignedDriverId ? "border-stone-700 bg-stone-900" : "border-orange-500/50 bg-orange-500/10"}`}>
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-2.5">
            <span className="text-3xl font-mono font-bold leading-none">{r.time}</span>
            <div>
              <div className="text-lg font-semibold leading-tight flex items-center gap-1.5"><Moon className="w-4 h-4 text-stone-500" />{destName(r)}</div>
              <div className="text-sm font-semibold text-orange-300 truncate">{r.djName || "—"}</div>
              <div className="text-xs text-stone-400">{r.passengerCount} Pers.</div>
            </div>
          </div>
          <StatusPill status={r.status} />
        </div>
        {r.meetingPoint && <div className="text-xs text-orange-300 mt-2 flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5" />{r.meetingPoint}</div>}
        {issue && <div className="text-xs text-red-300 mt-2">⚠ {(r.issues || []).filter(issueOpen).map((i) => i.type).join(", ")}</div>}
        ...
        <div className="flex items-center flex-wrap gap-2 mt-3">
          {drv ? (
            ...
            <button onClick={() => onAssign(r)} className="text-sm bg-orange-600 hover:bg-orange-500 text-white px-4 py-2.5 rounded-xl flex items-center gap-1.5"><Navigation className="w-4 h-4" />Fahrer zuweisen</button>
          )}
          {drv && flow?.next && <button onClick={() => onAssign(r)} className="text-sm text-stone-400 hover:text-stone-200 px-2 py-2.5">umteilen</button>}
          <button onClick={() => onWhatsApp(r)} className="text-sm bg-stone-800 hover:bg-stone-700 text-stone-100 px-3 py-2.5 rounded-xl flex items-center gap-1.5"><MessageSquare className="w-4 h-4" />Text</button>
          <button onClick={() => onEdit(r)} className="text-sm text-stone-500 hover:text-stone-300 ml-auto">öffnen</button>
        </div>
      </div>
    );
  };
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <h3 className="text-base text-stone-200 font-medium flex items-center gap-2"><Moon className="w-5 h-5 text-orange-400" />Rückfahrten · Nachtmodus</h3>
        <span className="text-xs text-stone-500">{fmtDate(day)}</span>
        <div className="flex items-center gap-1 bg-stone-900 border border-stone-800 rounded-lg p-0.5 ml-auto">
          <button onClick={() => setSort("time")} className={`text-xs px-2.5 py-1 rounded ${sort === "time" ? "bg-orange-600 text-white" : "text-stone-400"}`}>nach Zeit</button>
          <button onClick={() => setSort("dest")} className={`text-xs px-2.5 py-1 rounded ${sort === "dest" ? "bg-orange-600 text-white" : "text-stone-400"}`}>nach Ziel</button>
        </div>
      </div>

      {/* Wer ist vor Ort: pro Artist ein Button, zeigt fehlende oder schon geplante Rückfahrt */}
      {presence.length > 0 && (
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {covered.map((p) => (
              <button key={p.name} onClick={() => onEdit && onEdit(p.pendingReturn)}
                title="Rückfahrt bereits geplant — antippen zum Öffnen"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
        {[["Offen", open.length, open.length ? "text-orange-400" : "text-stone-300"], ["Unterwegs", active.length, "text-blue-400"], ["Erledigt", done.length, "text-emerald-400"], ["Probleme", problems.length, problems.length ? "text-red-400" : "text-stone-300"]].map(([l, v, c]) => (
          <div key={l} className="bg-stone-900 border border-stone-800 rounded-xl px-3 py-2.5 text-center">
      ...
      {returns.length === 0 && <div className="text-stone-500 text-sm py-12 text-center border border-dashed border-stone-800 rounded-xl">Keine Rückfahrten (Festival → Hotel/Airport) an diesem Tag.</div>}

      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <div className="xl:col-span-2 space-y-4">
          {open.length > 0 && (
            <div>
              <div className="text-xs uppercase tracking-wide text-orange-400 mb-2 flex items-center gap-1.5"><AlertTriangle className="w-3.5 h-3.5" />Offene Rückfahrten ohne Fahrer</div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">{sortRides(open).map((r) => <Tile key={r.id} r={r} big />)}</div>
            </div>
          )}
          {active.length > 0 && (
            <div>
              <div className="text-xs uppercase tracking-wide text-stone-400 mb-2">Zugeteilt / unterwegs</div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">{sortRides(active).map((r) => <Tile key={r.id} r={r} />)}</div>
            </div>
          )}
          {done.length > 0 && (
            <details className="group">
              <summary className="text-xs uppercase tracking-wide text-stone-500 cursor-pointer mb-2 flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" />Erledigte Rückfahrten ({done.length})</summary>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-2">
                {sortRides(done).map((r) => {
                  const drv = setup.drivers.find((d) => d.id === r.assignedDriverId);
                  return <div key={r.id} className="flex items-center gap-2 text-xs bg-stone-950/50 border border-stone-800 rounded-lg px-3 py-2 opacity-70">
                    <span className="font-mono text-stone-400">{r.time}</span><span className="text-stone-400">{destName(r)}</span>
                    <span className="text-stone-600 ml-auto">{drv ? drv.vehicleId : ""}</span>
                  </div>;
                })}
              </div>
            </details>
        ...
        <div className="space-y-3">
          <div className="bg-stone-900 border border-stone-800 rounded-xl p-3">
            <div className="text-xs text-stone-400 mb-2 flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5 text-orange-400" />Am Festival / frei ({atFestival.length})</div>
            <div className="space-y-1 max-h-[16rem] overflow-y-auto">
              ...
              {atFestival.length === 0 && <div className="text-xs text-stone-600">niemand frei am Festival</div>}
            </div>
          </div>
      ...
      {/* Timeline — wer wann besetzt ist, volle Breite */}
...
/* --------------- Große Leitstellenansicht / Überblick (Punkt 12) --------- */
function OverviewTab({ setup, dyn, day, setTab, onEdit, onAssign }) {
  const [, setTick] = useState(0);
  useEffect(() => { const t = setInterval(() => setTick((x) => x + 1), 30000); return () => clearInterval(t); }, []);
  const [ovRef, ovWidth] = useElementWidth();
  const sideBySide = ovWidth >= 820; // gemessen statt Viewport-Breakpoint (Artifact-Frame kann schmal sein)
  const ln = (id, c) => setup.locations.find((l) => l.id === id)?.short || c || "—";
  const now = dayNowMin(day);
  const live = now >= 0 && now < 90000;
  ...
  const openUnassigned = rides.filter((r) => !r.assignedDriverId && r.status !== "done");
  const airportPickups = rides.filter((r) => (r.fromId === "airport" || r.flightNo) && r.status !== "done").sort((a, b) => sortMin(a.scheduledArrival || a.time) - sortMin(b.scheduledArrival || b.time));
  const freeCount = setup.drivers.filter((d) => !computeDriverStats(setup, dyn, d.id, day).active).length;
  const flightsNeedAttention = airportPickups.some((r) => ["critical", "warn"].includes(flightAlert(r).level));
  const rep = dayReport(setup, dyn, day);
  const anstehend = rides.filter((r) => r.status === "planned").length; // noch nicht angenommen/gestartet

  // Kleiner, zahlenfreier Link-Chip (icon + label + optionaler Hinweis-Punkt)
  const Chip = ({ label, icon: I, go, alert }) => (
    <button onClick={() => setTab(go)}
      className="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full border border-stone-800 bg-stone-900 text-stone-300 hover:border-stone-600 hover:text-stone-100 transition">
      {I && <I className="w-3.5 h-3.5 text-stone-500" />}
      {label}
      {alert && <span className="w-1.5 h-1.5 rounded-full bg-orange-400" />}
    </button>
  );

  const CompactList = ({ title, icon: I, iconColor, items, empty, renderItem, moreGo }) => (
    <div className="bg-stone-900 border border-stone-800 rounded-xl p-2.5">
      <div className="text-[11px] uppercase tracking-wide text-stone-400 mb-1.5 flex items-center justify-between">
        <span className="flex items-center gap-1.5"><I className={`w-3.5 h-3.5 ${iconColor}`} />{title}</span>
        {moreGo && <button onClick={() => setTab(moreGo)} className="text-[10px] text-stone-600 hover:text-stone-300 normal-case">alle →</button>}
      </div>
      <div className="space-y-1 max-h-44 overflow-y-auto">
        {items.map(renderItem)}
        {items.length === 0 && <div className="text-[11px] text-stone-600 py-1">{empty}</div>}
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-4">
        <h3 className="text-lg font-semibold text-stone-100 flex items-center gap-2"><LayoutGrid className="w-5 h-5 text-orange-400" />Leitstelle · Überblick</h3>
        <span className="text-xs text-stone-500">{fmtDate(day)}{live ? ` · live ${fromMin(now)}` : ""}</span>
      </div>

      {/* Tagesfortschritt in Zahlen */}
      <div className="grid grid-cols-4 gap-2 mb-4">
        {[
          ["Erledigt", rep.done, "text-emerald-400", "board"],
          ["Offen", rep.open, "text-orange-400", "board"],
          ["Anstehend", anstehend, "text-sky-400", "board"],
          ["Gesamt", rep.total, "text-stone-100", "settings"],
        ].map(([label, value, color, go]) => (
          <button key={label} onClick={() => setTab(go)}
            className="bg-stone-900 border border-stone-800 hover:border-stone-600 rounded-xl px-3 py-2.5 text-center transition">
      ...
      <div ref={ovRef} className="grid gap-4" style={{ gridTemplateColumns: sideBySide ? "minmax(420px,1.7fr) minmax(260px,1fr)" : "1fr" }}>
        ...
        {/* schmale Seitenspalte: kompakte Listen (bewusst schmal gehalten) */}
        <div className="space-y-3">
          <CompactList title="Jetzt unterwegs" icon={Navigation} iconColor="text-blue-400" items={activeNow} empty="gerade niemand unterwegs"
            renderItem={(r) => {
              const drv = setup.drivers.find((d) => d.id === r.assignedDriverId);
              return (
          ...
          <CompactList title="Nächste 60 Minuten" icon={Clock} iconColor="text-orange-400" items={soon} empty="nichts in der nächsten Stunde"
            renderItem={(r) => {
              const drv = setup.drivers.find((d) => d.id === r.assignedDriverId);
              return (
                <button key={r.id} onClick={() => (drv ? onEdit(r) : onAssign(r))} className="w-full text-left flex items-center gap-1.5 text-xs bg-stone-950/50 hover:bg-stone-800 rounded-lg px-2 py-1.5">
                  <span className="font-mono text-stone-300">{r.time}</span>
                  <span className="text-stone-400 truncate">{ln(r.toId, r.toCustom)}</span>
                  {flightDelayed(r) && <Plane className="w-3 h-3 text-red-400 shrink-0" />}
                  {!drv && <span className="text-[9px] text-orange-400 ml-auto shrink-0">zuteilen</span>}
                </button>
              );
            }} />

          <CompactList title="Flughafen-Pickups" icon={Plane} iconColor="text-sky-400" items={airportPickups.slice(0, 8)} empty="keine Flughafen-Abholungen" moreGo="flights"
            renderItem={(r) => {
              const al = flightAlert(r);
              return (
                <button key={r.id} onClick={() => setTab("flights")} className={`w-full text-left flex items-center gap-1.5 text-xs bg-stone-950/50 hover:bg-stone-800 rounded-lg px-2 py-1.5 ${ALERT_ROW[al.level]}`}>
                  <span className="font-mono text-stone-300 truncate">{r.flightNo || r.time}</span>
                  <span className={`text-[10px] ${flightStyle(r.flightStatus).cls} px-1 py-0.5 rounded shrink-0`}>{flightStyle(r.flightStatus).l}</span>
                </button>
              );
            }} />
        </div>
      ...
      {/* Timeline — wer wann belegt ist, volle Breite */}
      <TimelineView setup={setup} dyn={dyn} day={day} onEdit={onEdit} />

      {/* Schnellzugriff, bewusst klein und ohne Zahlen — unten rechts */}
      <div className="flex items-center justify-end gap-2 mt-4">
        <Chip label="Aktiv: offene Fahrten" icon={Navigation} go="board" alert={openUnassigned.length > 0} />
        <Chip label="Freie Fahrer" icon={Users} go="drivers" />
        <Chip label="Pickups" icon={Plane} go="flights" alert={flightsNeedAttention} />
      </div>
    </div>
  );
}
...
function TimelinePage({ setup, dyn, day, onEdit, onAssign, updateDyn, by, onUndo }) {
  const ln = (id, c) => setup.locations.find((l) => l.id === id)?.short || c || "—";
  const [, setTick] = useState(0);
  useEffect(() => { const t = setInterval(() => setTick((x) => x + 1), 30000); return () => clearInterval(t); }, []);
