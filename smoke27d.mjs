...
const copy = "/tmp/sm27d-" + tag + ".jsx";
fs.writeFileSync(copy, fs.readFileSync(process.argv[2], "utf8") + "\nexport { TimelineView, BoardMiniMap, NoGpsSharingPanel, DriverRow };\n");
const out = "/home/claude/repo/.sm27d-" + tag + ".mjs";
...
for (const [n, C, p] of [
  ["TimelineView", m.TimelineView, { setup, dyn, day, onEdit(){} }],
  ["TimelineView (leer)", m.TimelineView, { setup, dyn: { rides: [], driverState: {} }, day, onEdit(){} }],
  ["BoardMiniMap (heute/live)", m.BoardMiniMap, { setup, dyn, day, onEdit(){} }],
  ["BoardMiniMap (sim/Regler)", m.BoardMiniMap, { setup, dyn, day: "2026-07-25", onEdit(){} }],
  ["NoGpsSharingPanel", m.NoGpsSharingPanel, { setup, dyn, day }],
  ["DriverRow (unterwegs)", m.DriverRow, { setup, driver: setup.drivers[0], stats }],
  ["DriverRow (frei/Car)", m.DriverRow, { setup, driver: setup.drivers[1], stats: { active: null, locNow: "hotel", count: 1, drivingMin: 60 } }],
]) {
  try { const h = renderToStaticMarkup(React.createElement(C, p));
    ...
    console.log("OK  " + n.padEnd(28) + String(h.length).padStart(5) + " Zeichen | MC-Treffer: " + String(mcT).padStart(3) + " | Classic-Reste: " + stone);
