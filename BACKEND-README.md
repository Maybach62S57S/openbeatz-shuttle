...
- **Kein offizielles API:** Life360 bietet keine öffentliche Entwickler-Schnittstelle. Das hier nutzt die intern von der Life360-App selbst verwendete, von der Community zurückentwickelte API (siehe ausführliche Warnung im Code). Kann jederzeit ohne Ankündigung brechen, ist vermutlich nicht durch Life360s AGB für automatisierten Zugriff gedeckt.
- **Sauberer Fallback bei Ausfall:** Jeder Fehler (Login schlägt fehl, Endpunkt geändert, Timeout) wird abgefangen und geloggt, es wird dann einfach nichts geschrieben. Die eigene Standortfreigabe der Fahrer läuft davon komplett unberührt weiter, das ist ein getrennter, unabhängiger Datenpfad.
- **Zuordnung:** pro Fahrer optional ein `life360_name` pflegen (Einstellungen → „Fahrer-Telefonnummern, Kennzeichen & PIN"), exakter Vor-+Nachname wie im Life360-Circle, case-insensitiv verglichen. Fahrer ohne gesetzten Namen werden übersprungen.
- **Freshness-Regel:** eine Life360-Position überschreibt eine vorhandene nur, wenn sie tatsächlich neuer ist — verhindert, dass eine ältere Life360-Position eine aktuellere Position aus der eigenen Standortfreigabe zurücksetzt.
- **Auslösung:** kein Vercel Cron (auf dem Hobby-Plan nur 1x/Tag möglich, zu selten). Stattdessen stößt die Leitstellen-Ansicht selbst alle 45s einen Sync an, solange eine Leitstelle eingeloggt ist (nicht bei Fahrern/Stage/Gast). Serverseitig zusätzlich auf ca. 20s gedrosselt, falls mehrere Leitstellen-Tabs gleichzeitig offen sind.
- **Einrichtung:** eigens für diesen Zweck ein Life360-Konto anlegen (NICHT das private Hauptkonto von dir oder einem Fahrer verwenden — bei Missbrauchserkennung durch Life360 könnte das Konto gesperrt werden, das würde dann auch die private Nutzung treffen), alle Fahrer in einen gemeinsamen Circle einladen, `LIFE360_EMAIL`/`LIFE360_PASSWORD` bei Vercel setzen (siehe `.env.example`).
- **Schema-Änderung nötig:** `drivers.life360_name` ist eine neue Spalte, `dispatcher_save_drivers` wurde entsprechend erweitert — falls das Schema schon einmal in Supabase ausgeführt wurde, den aktuellen `supabase-schema.sql`-Stand erneut im SQL Editor ausführen (alles mit `if not exists`/`create or replace`, gefahrlos erneut ausführbar).

## Echte Google-Maps-Ansicht (neu)
Zusätzlich zur kostenlosen Schema-Karte gibt es jetzt eine echte Kartenansicht (`LiveGoogleMap` in `shuttle-leitstelle.jsx`), die dieselben `driverState[driverId].gps`-Positionen direkt als Marker auf einer echten Google-Karte zeigt statt sie auf eine Schema-Strecke zu projizieren. Umschaltbar per Toggle „Schema-Karte" / „Google Maps" in der Karten-Ansicht (Desktop) und in der mobilen Leitstellen-Ansicht.
- **Bewusst nicht automatisch aktiv:** Das Google-Skript wird erst geladen, wenn eine Leitstelle aktiv auf „Google Maps" umschaltet, und danach nur einmal pro Browser-Sitzung (die Karte selbst wird nicht bei jedem 3s-Poll neu erzeugt, nur Marker-Positionen werden aktualisiert). Ohne gesetzten Key bleibt es automatisch bei der Schema-Karte, kein Fehlerzustand.
- **Zeigt nur Fahrer mit frischer GPS-Position** (jünger als `GPS_MAX_AGE_MS`, aktuell 3 Minuten), keine Offroute-Prüfung nötig, da hier die echten Koordinaten direkt auf einer echten Karte landen statt auf einer Schema-Strecke projiziert zu werden.
- **Einrichtung:** Google-Cloud-Projekt anlegen, Billing aktivieren (Pflicht, auch im Freikontingent), „Maps JavaScript API" aktivieren, Browser-API-Key erzeugen, per HTTP-Referrer auf eure Vercel-Domain(s) einschränken. Als `VITE_GOOGLE_MAPS_API_KEY` bei Vercel setzen (siehe `.env.example`) – bewusst clientseitig, das ist bei Google Maps so vorgesehen (Schutz über Referrer-Einschränkung, nicht über Geheimhaltung).
- **Kosten:** Google hat das frühere $200-Pauschalguthaben im März 2025 abgeschafft; aktuell (Stand 2026) gibt es pro SKU ein eigenes Freikontingent, für die Kartenanzeige (Dynamic Maps) rund 10.000 kostenlose „Map Loads" im Monat. Für den Festivalbetrieb (wenige Leitstellen-Geräte, Karte wird nicht bei jedem Poll neu geladen) bleibt ihr damit erfahrungsgemäß im Freikontingent – Google verlangt aber trotzdem eine hinterlegte Zahlungsmethode. Budget-Alerts in der Google Cloud Console einrichten schadet nicht.
...
## Direkte Schreibzugriffe per REST gesperrt (neu, Nachtrag 5)
Bisher erlaubten die RLS-Policies auf `drivers`/`settings`/`driver_state`/`rides`/`guest_tokens` jedem mit dem öffentlichen anon-Key (zwangsläufig im ausgelieferten JS-Bundle) direktes Schreiben per REST-API an der App komplett vorbei — unabhängig von `dispatcher_save_drivers`, `write_setup_if_unchanged`, `write_dyn_if_unchanged` und allen Gast-RPCs. Jetzt gesperrt: alle Schreibzugriffe laufen ausschließlich über diese RPCs (SECURITY DEFINER, umgehen RLS und funktionieren unverändert weiter — kein Funktionsverlust). `rides`/`driver_state` (laut Nachtrag oben bereits vollständig unbenutzte Alt-Tabellen) und `guest_tokens` sind zusätzlich auch für direktes **Lesen** per anon-Key gesperrt, da nichts Legitimes das braucht (guest_tokens enthält sonst quasi eine Generalliste aller gültigen Gast-Link-Tokens). **Noch offen, bewusst nicht Teil dieses Nachtrags:** `drivers.pin`, `settings.config.defaultPin`/`stagePin` sowie die PINs in `settings.dispatchers` sind weiterhin über den anon-Key per normalem Lesezugriff auslesbar (der aktuelle Login vergleicht PINs komplett im Browser gegen diese geladenen Werte) — siehe eigener Abschnitt weiter unten, das ist ein größerer Umbau (server-seitige PIN-Prüfung), der den Login-Mechanismus aller drei Rollen betrifft.
