...
- **"Kein Live-Standort"-Übersicht:** in der Karten-Ansicht (Desktop und mobil) erscheint automatisch ein Hinweis-Panel mit allen Fahrern, die heute eine Fahrt haben, aber gerade keine frische GPS-Position teilen — inkl. Anruf-Button, damit proaktiv nachgehakt werden kann. Erscheint nur bei Bedarf, ist leer/unsichtbar, wenn alle aktiven Fahrer teilen.

## Life360-GPS-Sync (ausprobiert, wieder entfernt)
War als zusätzliche, optionale GPS-Quelle neben der eigenen Standortfreigabe geplant. Gegen einen echten Account getestet (12.07.2026): Login schlägt mit HTTP 403 fehl, Antwort ist eine Cloudflare-Block-Seite (Header `server: cloudflare`, `cf-ray` gesetzt) — die Anfrage wird schon vor Life360s eigentlicher Login-Prüfung geblockt, unabhängig von Zugangsdaten oder Client-Token (zwei verschiedene, dokumentierte Tokens getestet, identischer Fehler). Bewusst nicht weiterverfolgt: das Umgehen einer erkennbaren Bot-Sperre wäre nötig, um das doch zum Laufen zu bringen, das machen wir nicht.

Code lag in `api/life360-sync.js` (mit dem 45s-Trigger aus der Leitstellen-Ansicht und dem `life360_name`-Feld pro Fahrer verdrahtet), wurde am 12.07.2026 wieder vollständig entfernt, um die Vercel-Logs während des Festivals nicht mit wiederkehrenden 403-Fehlern zu verrauschen. Steht in der Git-Historie, falls es später nochmal versucht werden soll (z. B. falls Life360 seine Sperre lockert). Die eigene Standortfreigabe der Fahrer (`useDriverLocationSharing`) ist davon unberührt und bleibt die alleinige GPS-Quelle.
